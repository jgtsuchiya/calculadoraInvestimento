const isNonEmptyArray = (arrayElement) => {
    return Array.isArray(arrayElement)
...
export const createTable = (columnsArray, dataArray, tableId) => {
    if (
        ...
        throw new Error('Dados incorretos, insira um array, os dados e o ID correto');
    }
    ...
    if (
        ...
        throw new Error('ID não correponde a tabela');
    }
...
function createTableHeader(tableReference, columnsArray) {
    ...
    for (const tableColumnObject of columnsArray) {
        const headerElement = `<th class='text-center'>
            ${tableColumnObject.columnLabel}</th>`;
    ...
    for (const [itemIndex, tableItem] of tableItems.entries()) {
        ...
        for (const tableColumn of columnsArray) {
            ...
            tableRow.innerHTML += `<td class='text-center'>
                ${formatFn(tableItem[tableColumn.accessor])}</td>`;
        }
